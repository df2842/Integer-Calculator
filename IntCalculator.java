...
public class IntCalculator {
    ...
    {
        MyStack<Character> postfixStack = new MyArrayList<>();
        ...
        for (int i = 0; i < infixExpression.length(); i++)
        {
            ...
            {
                if (postfixStringBuilder.lastIndexOf(" ") != postfixStringBuilder.length() - 1)
                {
                    postfixStringBuilder.append(' ');
                }
                while (!postfixStack.isEmpty() && postfixStack.peek() != '(')
                {
                    postfixStringBuilder.append(postfixStack.pop()).append(' ');
                }
                postfixStack.pop();
            }
            ...
            else if (isBinaryOperator(ch))
            {
                ...
                while (!postfixStack.isEmpty() && (isLeftAssociative(ch) || ch != postfixStack.peek()) && precedence(ch) <= precedence(postfixStack.peek()))
                {
                    postfixStringBuilder.append(postfixStack.pop()).append(' ');
                }
                postfixStack.push(ch);
        ...
        if (!postfixStack.isEmpty())
        {
            ...
            postfixStringBuilder.append(postfixStack.pop());
        }
        if (postfixStringBuilder.lastIndexOf(" ") == postfixStringBuilder.length() - 1)
        {
            postfixStringBuilder.deleteCharAt(postfixStringBuilder.length() - 1);
        }

        return this.postfixExpression = postfixStringBuilder.toString();
    }
    ...
    {
        MyStack<Integer> evaluateStack = new MyArrayList<>();
        String postfixExpressionCopy = this.postfixExpression + " ";

        while (!postfixExpressionCopy.isEmpty())
        {
            String oper = postfixExpressionCopy.substring(0, postfixExpressionCopy.indexOf(" "));
            postfixExpressionCopy = postfixExpressionCopy.substring(postfixExpressionCopy.indexOf(" ") + 1);

            try
            {
                evaluateStack.push(Integer.parseInt(oper));
            }
            catch (Exception e)
            {
                switch (oper)
                {
                    case "~" -> evaluateStack.push(-evaluateStack.pop());
                    case "^" ->
                    {
                        int operatorTwo = evaluateStack.pop();
                        int operatorOne = evaluateStack.pop();
                        if (operatorOne == 0 && operatorTwo == 0)
                        {
                            throw new IllegalArgumentException("Cannot evaluate expression, 0^0 is undefined.");
                        }
                        evaluateStack.push((int) Math.pow(operatorOne, operatorTwo));
                    }
                    case "x" -> evaluateStack.push(evaluateStack.pop() * evaluateStack.pop());
                    case "/" ->
                    {
                        int operatorTwo = evaluateStack.pop();
                        if (operatorTwo == 0)
                        ...
                        evaluateStack.push(evaluateStack.pop() / operatorTwo);
                    }
                    case "%" ->
                    {
                        int operatorTwo = evaluateStack.pop();
                    ...
                    case "+" -> evaluateStack.push(evaluateStack.pop() + evaluateStack.pop());
                    case "-" -> evaluateStack.push(-evaluateStack.pop() + evaluateStack.pop());
                }
            }
        }
